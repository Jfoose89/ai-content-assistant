...
A distributed microservices system built with ASP.NET Core (.NET 9), consisting of two cooperating Web APIs that generate atmospheric D&D content using AI. The system uses a bundled local SRD data file to enrich prompts with real stat block data before sending them to HuggingFace for generation.
How It Works

A client sends a POST request to Service A with a D&D category and optional SRD reference
Service A looks up real SRD stat block data from its bundled local JSON file (srd-data.json)
...
Service A depends on one external service:
...
monster — Atmospheric monster descriptions for DMs. Fetches real stat block from srd-data.json if an SRD reference is provided.
spell — Vivid spell effect descriptions. Fetches real spell details from srd-data.json if an SRD reference is provided.
...
POST https://localhost:5001/api/DndContent
...
Service A will look up the Aboleth's real stat block from srd-data.json and build an enriched prompt before calling HuggingFace.
...
srd-data.json — Bundled SRD stat block data (monsters and spells)
...
DndSrdService.cs — Reads SRD data from the bundled local JSON file
